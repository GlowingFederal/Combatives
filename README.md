...
- Dedicated crawl keybind (`C` by default) with common-preInit Combatives networking for authoritative server-side crawl state; client-side access to the Minecraft singleton, key client fields, and key binding pressed state uses SRG-first helpers so production jars do not call an unmapped `Minecraft.getMinecraft()` name
