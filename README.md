...
- Dedicated crawl keybind (`C` by default) with common-preInit Combatives networking for authoritative server-side crawl state; client-side access to the Minecraft singleton and key client fields uses an SRG-first helper so production jars do not call an unmapped `Minecraft.getMinecraft()` name
