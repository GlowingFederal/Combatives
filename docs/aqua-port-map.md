...
| `proxy/ClientProxy` key event registration | `proxy/ClientProxy` and `client/ClientMovementInputHandler` | Rewritten as client-tick input tracking so press and release states are both sent to the server; startup logging now confirms the client proxy and keybind registration path. |
| `mixins/early/minecraft/client/EntityPlayerSPMixin` sprint/swim state handling | `mixin/EntityPlayerSPMixin` | Ported the missing client-side water sprint persistence needed to stop vanilla 1.7.10 from cancelling sprint, which immediately cancelled Aqua-style swimming. |
...
| `EntityRendererMixin`, `ItemRendererMixin`, `RenderPlayerMixin`, `ModelBipedMixin`, `PlayerControllerMPMixin`, `EntityClientPlayerMPMixin`, `EntityOtherPlayerMPMixin` rendering/remote-player portions | Rendering/camera overhauls are non-goals for this PR. The local-player sprint/swim state portion is now ported separately; rendering/camera/remote-player visual adjustments remain out of scope. |
