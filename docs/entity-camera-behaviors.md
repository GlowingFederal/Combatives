...
- **Landing** tracks the fastest sampled downward velocity until the grounded transition, then combines that velocity, sampled vertical acceleration, and fall distance as a secondary severity signal. Internally it drives a short impact/compression hold followed by a critically damped recovery, with slight pitch and acceleration-directed micro-roll. This prevents the recovery from reversing the camera immediately after contact.
