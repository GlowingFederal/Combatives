...
public class PacketPlayerGeometryS2C implements IMessage {
    private int entityId, rawSize;
    private float widthScale, heightScale, eyeScale;
    private boolean fromMpm;
    public PacketPlayerGeometryS2C() { }
    public PacketPlayerGeometryS2C(int entityId, MpmCompatibility.Geometry geometry) {
    ...
    @Override public void fromBytes(ByteBuf buf) {
        entityId = buf.readInt(); rawSize = buf.readInt(); widthScale = buf.readFloat();
        heightScale = buf.readFloat(); eyeScale = buf.readFloat(); fromMpm = buf.readBoolean();
    }
    @Override public void toBytes(ByteBuf buf) {
        buf.writeInt(entityId); buf.writeInt(rawSize); buf.writeFloat(widthScale);
